...
import { flagReport, getModerationQueue, handleModerationAction } from "../../../lib/store.ts";
import { flagReportSchema, moderationActionSchema } from "../../../lib/validators.ts";

export async function GET(request: Request) {
  try {
    assertAdminTokenHeader(request);
    return ok(await getModerationQueue(), {
...
export async function POST(request: Request) {
  try {
    ...
    return ok(await flagReport(input, { request }), {
...
export async function PATCH(request: Request) {
  try {
    ...
    return ok(await handleModerationAction(input), {
      policy: "운영자 처리 사유와 처리 시각을 저장합니다.",
    });
  } catch (error) {
    return fail(error);
  }
